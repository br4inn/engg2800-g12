...
extern struct ReceivedParams received_params;
...
void process_binary_data(char *binary_data) {
	...
	strncpy(year_string, binary_data + 20, 12);
     ...
     // Store the values in the struct (received_params)
    received_params.hour = hour;
    received_params.minute = minute;
    received_params.day = day;
    received_params.month = month;
    received_params.year = year;
